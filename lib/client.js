	...
	factory: (require) => {
		...
		function FileBrowserPanel(props) {
			...
			if (workspaceState && Array.isArray(workspaceState.items)) {
				...
				if (!workspacePath && workspaceState.items.length > 0 && workspaceState.items[0] && workspaceState.items[0].path) {
					workspacePath = workspaceState.items[0].path;
				}
			}
			...
			const load = useCallback(async (target) => {
				...
				try {
					...
					setDirs(r.dirs || []);
					setFiles(r.files || []);
					rememberFolder(workspacePath, r.path);
				} catch (e) {
			...
			// 挂载即恢复上次浏览的文件夹（按工作区记忆）；没有记忆则加载工作区根目录。
			// 这样重启/切会话后右侧直接出内容，不用等工作区列表也出现空白。
			useEffect(() => {
				if (path) return;
				const saved = workspacePath ? lastFolders[workspacePath] : lastFolders["_"];
				const target = saved || workspacePath;
